    ...
    **/
    ...
    function tmpl$5($api, $cmp, $slotset, $ctx) {
      ...
      return [api_element("div", {
        ...
        key: 11
      }, [api_dynamic($cmp.markersTitle), api_text(" ("), api_dynamic($cmp.state.coordinates.length), api_text(")")])]), api_element("ul", {
        classMap: {
      ...
      }, api_iterator($cmp.state.coordinates, function (coordinate) {
        return api_element("li", {
          classMap: {
            "slds-coordinates__item": true
          },
    ...
    class LightningMap extends lwc.LightningElement {
      constructor(...args) {
        ...
        this.state = {
          ...
          markersTitle: i18n$1.coordinatesTitleString
        };
        this.privateZoomLevel = null;
        this.privateCenter = null;
        this.privateMarkers = null;
        this.privateCoordinateItems = [];
        this.mapDomain = `https://maps${getCoreInfo().untrustedContentDomain}:${getCoreInfo().securePort}`;
        this.mapSrc = `${this.mapDomain}/lightningmaps/mapsloader?resource=primitiveMap&version=${getCoreInfo().internalAppVersion}`;
      ...
      set center(value) {
        ...
        const computedCenter = this.primitivifyMarker(lwc.unwrap(this.center));
        this.postToIframe({
      ...
      initMarkers(mapMarkers) {
        ...
        for (i; i < mapMarkersLength; i++) {
          key = guid();
          coordinate = lwc.unwrap(mapMarkers[i]);
          coordinate.key = key;
        ...
        const markers = this.state.coordinates.map(marker => this.primitivifyMarker(marker));
        this.postToIframe({
          markers
        });
      }
      ...
      handleCoordinateClick(event) {
        ...
        this.state.activeMarkerId = key; // unselect other child coordinateitems
      ...
      handleIframeLoad(event) {
        const center = this.center ? this.primitivifyMarker(lwc.unwrap(this.center)) : null;
        const zoomLevel = this.zoomLevel;
        const markers = lwc.unwrap(this.state.coordinates).map(marker => this.primitivifyMarker(marker));
    ...
    function tmpl$6($api, $cmp, $slotset, $ctx) {
      ...
      return [api_custom_element("lightning-card", _lightningCard, {
        ...
        key: 2
      }, [api_custom_element("lightning-map", _lightningMap, {
        props: {
    ...
    tmpl$6.stylesheetTokens = {
      hostAttribute: "c-map_map-host",
      shadowAttribute: "c-map_map"
    };
    ...
    registerWireService(lwc.register);
        ...
        const element = lwc.createElement('c-map', { is: main, fallback: true });
